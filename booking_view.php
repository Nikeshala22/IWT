...
<div class="container">
    ...
    $select_booking = $con->query("SELECT * FROM `bookingdetails`");

    // Check if any bookings exist
    if ($select_booking->num_rows > 0) {
        // Fetch the first booking (assuming you only want to edit one booking at a time)
        $fetch_booking = $select_booking->fetch_assoc();

        // Display the booking details in an edit form
        ?>
        <h2> Land Booking</h2>
        <form action="booking_edit.php?id=<?= $fetch_booking['id']; ?>" method="post">
            ...
            <a href="booking.php?post_id=<?= $fetch_booking['id']; ?>" class="btn">Go Back</a>
        </form>
        <?php
    } else {
        // Display a message if no bookings exist
        echo '<p>No bookings added yet! <a href="booking.php" class="btn" style="margin-top: 1.5rem;">Add Booking</a></p>';
    }
    ?>
</div>
